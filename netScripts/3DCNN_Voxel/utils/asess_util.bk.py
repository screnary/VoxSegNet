...
def assess_Unet_result(pred_val, current_seg, loss_Unet_val, part_num, acc_ops):
    ...
    cur_seg_new = current_seg - 1.0  # pred is from 0 value, but seg gt is from 1 value (0 for the back)
    mask = cur_seg_new != -1
    mask = mask.astype(np.float32)
    correct = np.sum(pred_val == cur_seg_new, axis=(1,2,3))
    ...
    intersect_mask = np.int32(pred_val == cur_seg_new)  # [B,V,V,V]
    # pdb.set_trace()
...
def assign_mask_to_vox(mask_pred, box, part_id, vox_size, part_num):
    ...
    vox_seg = np.zeros((vox_size,vox_size,vox_size,part_num))
