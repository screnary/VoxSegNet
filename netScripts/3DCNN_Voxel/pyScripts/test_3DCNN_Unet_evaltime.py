...
parser.add_argument('--model', default='3DCNN_Unet', help='Model name: boundary_model,3DCNN_model [default: 3DCNN_model]')
parser.add_argument('--log_dir', default='log-3DCNN_Unet', help='Log dir [default: log]')
...
checkpoint_dir = os.path.join(FLAGS.log_dir+g_.Data_suf, str(VOX_SIZE),
    CLASSNAME+'-withBG', 'trained_models')
...
if not FLAGS.ignore_Empty:
    output_dir = os.path.join(
            BASE_DIR, './test_results_' + g_.Data_suf + FLAGS.model,
            CLASSNAME+'-withBG-evaltime')
...
# pdb.set_trace()
output_Box = False
if not os.path.exists(os.path.join(BASE_DIR, './test_results_' + g_.Data_suf + FLAGS.model)):
    os.mkdir(os.path.join(BASE_DIR, './test_results_' + g_.Data_suf + FLAGS.model))
...
def evaluate(num_votes):
    ...
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        if FLAGS.ignore_Empty:
            ...
            pred, feature = MODEL.get_model(vol_ph, is_training_ph,
                    part_num+1)
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            # log_string('model_%d, invalid voxels:%.2f, iou_vol:%.4f, iou_pts:%.4f' %
                # (shape_idx, invalid_vox_num, avg_iou, avg_iou_pts))
