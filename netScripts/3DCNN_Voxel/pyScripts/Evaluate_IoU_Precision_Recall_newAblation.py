""" WZJ:20180717-Evaluate Results: AP, Recall, Accuracy, IoU--Atrous_Encoder"""
...
parser.add_argument('--dump_dir', default='dump-PRvalues', help='dump folder path [dump]')
...
GPU_INDEX = FLAGS.gpu
# sys.path.append(os.path.join(FLAGS.log_dir+g_.Data_suf, str(VOX_SIZE),
#     CLASSNAME+'-withBG'+'-ABlock'+str(FLAGS.atrous_block_num)))  # model path
MODEL = importlib.import_module(FLAGS.model)  # import network module

out_base_dir = os.path.join(BASE_DIR, './Evaluation' + g_.Data_suf + FLAGS.model + '-newAblation')
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    total_set_precision_pts = 0.0
    total_set_recall_pts = 0.0
    ...
    for fn in range(len(TEST_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            plb_pred = vol_label_projection_to_pts(pts, seg_pred_val)
            if not FLAGS.ignore_Empty:
                # 0 label is for predicted empty voxles (background label)
                plb_gt = plb_gt + 1.0
                plb_pred[plb_pred==-1]=0.0
            ...
            total_iou_pts = 0.0
            iou_log_pts = 'iou_pts\n'  # iou details string
            total_precision_pts = 0.0
            total_recall_pts = 0.0
            pr_log_pts = 'Precision Recall\n'

            """ compute IoU, Precision and Recall
            True Positive (TP): # items correctly labeled as belonging to the positive class (eqv. with hit)
            False Positive(FP): # items incorrectly labeled as belonging to the positive class (eqv. with false alarm, Type I error)
            False Negative(FN): # items which were not labeled as belonging to the positive class but should have been (eqv. with miss, Type I error)
            True Negative (TN): # items which were not labeled as belonging to the positive class, and that is correct (eqv. with correct rejection)
            """
            for oid in iou_oids:
                n_pred = np.sum(plb_pred == oid)  # only the valid grids' pred, TP+FP
                # n_pred = np.sum(seg_pred_val == oid)
                n_gt = np.sum(plb_gt == oid)
                n_intersect = np.sum(np.int32(plb_pred == oid) * mask)  # TP
                n_union = n_pred + n_gt - n_intersect
                ...
                if n_pred == 0:
                    ...
                    precision_part = n_intersect*1.0 / n_pred  # TP/(TP+FP)
                if n_gt == 0:
                    recall_part = 1.0
                else:
                    recall_part = n_intersect*1.0 / n_gt  # TP/(TP+FN)
                pr_log_pts += '\tpart_' + str(oid) + '-precision: ' + str(precision_part) + ' -recall: ' + str(recall_part) + '\tgt_part_ratio: ' + str(n_gt*1.0/len(plb_gt)) + '\n'
                total_precision_pts += precision_part
                total_recall_pts += recall_part

            # per instance eval
            avg_iou_pts = total_iou_pts / len(iou_oids)  # average iou across parts, for one object
            avg_precision_pts = total_precision_pts / len(iou_oids)
            avg_recall_pts = total_recall_pts / len(iou_oids)

            # eval across shapes set
            total_acc_iou_pts += avg_iou_pts
            total_set_precision_pts += avg_precision_pts
            total_set_recall_pts += avg_recall_pts

            acc_per_instance_pts = np.sum(mask) / len(plb_gt)
            total_accuracy_pts += acc_per_instance_pts

            log_string('model_%d, iou_pts:%.4f, precision:%.4f, recall:%.4f' %
                (shape_idx, avg_iou_pts, avg_precision_pts, avg_recall_pts))
            log_string(pr_log_pts)
    ...
    log_string('Voxel version detail: **************\n')
    ...
    log_string('total Precision pts: %f' % (total_set_precision_pts / total_seen))
    log_string('total Recall pts: %f' % (total_set_recall_pts / total_seen))
